...
Sprite::Sprite(float_t xPosition, float_t yPosition, uint16_t width, uint16_t height, char8_t* spriteName, uint8_t spriteLayer)
{
	...
	mWidth = width;
	mHeight = height;
	mSpriteTextureID = SOIL_load_OGL_texture(spriteName, SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT);
...
void Sprite::render()
{
	...
	glVertex3f(getPosition()->x, getPosition()->y + mHeight, 0.0);

	// Bottom Left
	glTexCoord2f(0.0f, 0.0f);
	glVertex3f(getPosition()->x, getPosition()->y, 0.0);

	// Bottom Right
	glTexCoord2f(1.0f, 0.0f);
	glVertex3f(getPosition()->x + mWidth, getPosition()->y, 0.0);

	// Top Right
	glTexCoord2f(1.0f, 1.0f);
	glVertex3f(getPosition()->x + mWidth, getPosition()->y + mHeight, 0.0);
